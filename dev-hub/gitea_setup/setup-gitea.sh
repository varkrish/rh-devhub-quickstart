...
update_yaml_fields() {
    ...
    local gitea_access_token=$(head -1 /secrets/.accesstoken)
    ...
    yq eval ".integrations.gitea[0].password = \"$gitea_access_token\"" "$output_yaml" -i

    yq eval ".integrations.gitlab[0].password = \"$gitea_access_token\"" "$output_yaml" -i
}
